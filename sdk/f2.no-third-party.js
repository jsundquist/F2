	...
	var _loadApps = function(appConfigs, appManifest) {
		...
		var _loadStyles = function(styles, cb) {
			...
			if (_config.loadStyles) {
				_config.loadStyles(styles, function() {
					cb();
				});
			}
			else {
				...
				jQuery.each(styles, function(i, path) {
					if (useCreateStyleSheet) {
						document.createStyleSheet(path);
					}
					else {
						stylesFragment.push('<link rel="stylesheet" type="text/css" href="' + path + '"/>');
					}
				});
		...
		var _loadScripts = function(scripts, inlines, cb) {
			...
			if (_config.loadScripts) {
				_config.loadScripts(scripts, inlines, function() {
					cb();
				});
			}
			else {
				// Fn to load inline scripts
				var evalInlines = function(inlines) {
					jQuery.each(inlines, function(i, code) {
						try {
							eval(code);
						}
						catch (exception) {
					...
					while (scripts.length) {
						...
						jQuery.ajax({
							...
							error: function(jqxhr, settings, exception) {
								F2.log(['Failed to load script (' + path + ')', exception.toString()]);
							}
						});
					}
				}
		...
		var _loadHtml = function(apps) {
			jQuery.each(apps, function(i, a) {
				if (!_bUsesAppHandlers) {
					...
					root.className += F2.Constants.Css.APP_CONTAINER + ' ' + appId;
				}
